...
func TestPlanStdinBase64(t *testing.T) {
	mock := testutil.NewMockProvider()
	inputData := `{"version":1,"provider":"aws-secretsmanager","secrets":[{"key":"/stdin-b64","value":"env-secret"}]}`
	encoded := base64.StdEncoding.EncodeToString([]byte(inputData))
	...
	cli.SetStdinReader(bytes.NewReader([]byte(encoded)))
	t.Cleanup(func() {
		cli.ResetProviderFactory()
		cli.ResetStdinReader()
		cli.ResetRunConfig()
